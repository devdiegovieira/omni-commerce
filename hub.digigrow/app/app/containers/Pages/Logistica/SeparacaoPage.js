...
import SeparacaoFilter from './SeparacaoFilter';
import SeparacaoActions from './SeparacaoActions';
import { useSnackbar } from 'notistack';
import ButtonDefault from '../../../components/Button/ButtonDefault';
import SellerSelectToolbar from '../Sale/SaleSelectToolbar';
import SaleUpXml from '../Sale/SaleUpXml';



function SeparacaoPage(props) {
  const { intl } = props;
      ...
      options: {
        customBodyRenderLite: (dataIndex) => {
          ...
          return (
            <>
              <Grid >
                <ButtonDefault icon={'upload'} label={'Importa XML'} onClick={() => { data.xmlOpen = !data.xmlOpen; setGridData([...gridData]) }} />
                {data.xmlOpen && (<Grid item style={{ marginTop: -20 }}><SaleUpXml orderId={data.orderId} /></Grid>)}
              </Grid>
            </>
          );
        }
        ...
        customBodyRenderLite: (dataIndex) => {
          ...
          return (
            <Switch
              name="printed"
              color="primary"
              checked={data.printed ? true : false}
            />
          );
        }
      }
...
SeparacaoPage.propTypes = {
  intl: intlShape.isRequired
};

export default injectIntl(SeparacaoPage);
