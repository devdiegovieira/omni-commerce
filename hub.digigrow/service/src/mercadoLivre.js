...
const { meliOrderToDigigrowOrder, getInvoicedMeliFulfillment } = require('./../../lib/data/order');
        ...
        if (!orderHub) {
          ...
          if (publishies && publishies.length > 1 && distictSellers.length > 1) {
            ...
            if (publishies && publishies.length) {
              ...
              // Save order on hub
              let digiOrder = await meliOrderToDigigrowOrder(orderMeli, queue, sellerId, service.platformId, db);
