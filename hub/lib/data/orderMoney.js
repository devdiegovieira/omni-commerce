...
const getOrderMoneyReport = async (db, headers, day, year, month, paymentStatus) => {
  ...
  try {
    ...
    let moneyReport = {};
    let filter = {
      paymentStatus: status ? status : ['concluded'],
      closedDate,
      sellerId: [sellerId],
      offset: 0,
      limit: 500
    ...
    do {
      moneyList = await getOrderMoneyList(db, { ...filter });
      filter.offset = filter.offset + 500;

      let statusList = [...new Set(moneyList.list.map(item => item.paymentStatus))]

      for (let status of statusList) {
        if (!moneyReport[status]) moneyReport[status] = [];
        moneyReport[status].push(...moneyList.list.filter(f => f.paymentStatus == status));
      }


    } while (moneyList.list.length > 0);
    ...
    for (let status in moneyReport) {
      Array.isArray(moneyReport[status]) && moneyReport[status].map((m, i, arr) => {
        arr[i] = {
          ...
          shipping: m.shipping,
          Cliente: m.buyerName,
          SomaProdutos: m.payBack ? m.productSum * -1 : m.productSum,
          taxaMeli: m.saleFee,
          taxa: m.digiFee,
          freteCliente: m.payBack ? m.freightClient * -1 : m.freightClient,
          freteLoja: m.payBack ? m.freightSeller * -1 : m.freightSeller,
