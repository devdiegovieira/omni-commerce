...
const { upsertQueueSku } = require("./queue");
const { downloadURI, deleteFileS3, uploadFileS3 } = require("../util/storage");
...
const updateSkuXPublish = async (db, sku, gtin, price, stock, sellerId) => {
...
const upsertProdAndVars = async (db, prod, seller) => {
  ...
  if (prod.depth) productSet['depth'] = Number(prod.depth);
  ...
  for (let variation of prod.variations) {
    ...
    await updateSkuXPublish(db, variation.sku, variation.gtin, variation.price, variation.stock, variationSet.sellerId)    
  }
...
const validateProdAndVars = (prod, skus, seller, user, checkNew) => {
  try {
    ...
    if (!prod.sku) throw 'Código SKU é obrigatório';
    ...
    for (let variation of prod.variations) {
      try {
        let varExist = skus.find(f => f.sku == variation.sku || f.gtin == variation.gtin);
        ...
        if (!variation.gtin) throw 'Código GTIN/EAN é obrigatório';
        if (prod.variations.filter(f => f.gtin == variation.gtin).length > 1) throw 'Não é possível cadastrar mais de uma variação com o mesmo GTIN/EAN';

        if (varExist && (checkNew || variation.isNew)) throw 'Código de variação ou GTIN/EAN já cadastrado para esta empresa';
...
const distinctValues = (prods, field = 'sku') => {
  let codes = new Set();
  prods.map(prod => {
    if (prod[field])
      codes.add(prod[field]);

    if (Array.isArray(prod.variations))
      prod.variations.map(vari => { if (vari[field]) codes.add(vari[field]) })
  })
...
const upsertProducts = async (db, products, user, checkNew = false) => {
  ...
  //somente empresas que o usuário pode editar
  ...
  for (prodsBySeller in prodSellers) {
    try {
      ...
      let skus = await skuColl.find({
        sellerId: seller._id,
        $or: [
          {sku: { $in: distinctValues(prodSellers[prodsBySeller], 'sku') }},           
          {gtin: { $in: distinctValues(prodSellers[prodsBySeller], 'gtin') }}
        ] 
      }).toArray();
...
const getSkuList = async (db, filter, user) => {
  ...
  let variations = await skuCollection.find({ productId: { $in: products.map(m => m.sku) }, sellerId: { $in: products.map(m => m.sellerId) } }).toArray();
...
const updateSkuStock = async (db, userName, sku, seller, quantity) => {
  ...
  await upsertQueueSku(db, userName, updateSku);
}

const getDefaultData = ( gtin ) => {

  // return {}
}

module.exports = { getPriceWithRule, updateSkuStock, upsertProducts, getSkuList, productReport, filterSkuList, uploadCards, uploadErrorList, productReport, getDefaultData };
