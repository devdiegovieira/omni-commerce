...
const meliOrderToOrder = async (meliOrder, queue, sellerId, platformId, db) => {
  ...
  let grossMeli = toFixed(items.reduce((n, { gross }) => n + gross, 0), 2);
  let sale_fee = toFixed(items.reduce((n, { saleFee }) => n + saleFee, 0), 2);
...
const getOrderDetail = async (db, user, orderId) => {
  ...
  for (let item of orderDetail.items) {
    ...
    if(item.skuImage)item.skuImage = skuXPublish.images[0];
...
const getOrderExpeditionList = async (db, user, offset, limit, status, data) => {
  ...
  if (data.dateclosedfrom || data.dateclosedto) {
    ...
    if (data.dateclosedto) filterMongo.dateClosed.$lt = new Date(data.dateclosedto);
  }
...
const getOrderPickingList = async (db, user, orders, printed) => {
  ...
  return orderPickigList.map(m => {
    return {
      ...
      items: m.items.map(mm => { return { sku: mm.sku, title: mm.title, quantity: mm.amount, address: sku.find(f => f.sku == mm.sku).address } })
