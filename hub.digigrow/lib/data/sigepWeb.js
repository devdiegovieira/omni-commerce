...
const orderToSigepWebXML = async (db, orderArray) => {
  ...
  let listaObjetosPostais = orderArray.map(m => {
    return {
      ...
      complemento: m.shipping.comment,
  ...
  const bairroRemetente = seller[0].neighborhood;
  const cepRemetente = seller[0].cep;
  const cidadeRemetente = seller[0].city;
  const ufRemetente = seller[0].state;
  const telefoneRemetente = seller[0].phone;
  const faxRemetente = seller[0].phone;
  ...
  let header =
    ...
    <correioslog>
      ...
      <remetente>
        ...
        <logradouro_remetente>${logradouroRemetente}</logradouro_remetente>
  ...
  listaObjetosPostais.forEach((objetoPostal, index) => {
    ...
    postalObjects.push(`
      <objeto_postal>
        ...
        <cubagem>${objetoPostal.cubagem}</cubagem>
        <peso>${objetoPostal.peso}</peso>
        <rt1/>
        <rt2/>
        <destinatario>
        ...
        <nacional>
          ...
          <valor_a_cobrar>${objetoPostal.valorCobrar}</valor_a_cobrar>
        </nacional>
        <servico_adicional>
          <codigo_servico_adicional>${objetoPostal.servicoAdicional}</codigo_servico_adicional>
