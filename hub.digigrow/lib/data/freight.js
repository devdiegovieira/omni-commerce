...
const closePlp = async (db, body) => {
  try {
    ...
    let order = await orderColl.find({ $or: [{ externalId: { $in: body } }, { packId: { $in: body } }] }).toArray();
    ...
    order.map(m => { if (m.shipping.mode == 'me1') { listingOrderCorreios.push(m) } else { listingOrderMercadoEnvios.push(m) } });

    let xml;

    if (listingOrderCorreios.length > 0) {
      let xmlOrderToSigepWeb = await orderToSigepWebXML(db, listingOrderCorreios);
      xml = xmlOrderToSigepWeb
      ...
      await orderColl.updateMany({ $or: [{ externalId: { $in: listOrder.map(m => m.order) } }, { packId: { $in: listOrder.map(m => m.order) } }] }, { $set: { 'shipping.shippedDate': shippedDate, status: 'shipped' } })
      reportPlp.push(listingOrderCorreios.map(m => { return { ...m, plpCorreios: plpId } }));
    }
