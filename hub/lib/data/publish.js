...
const upsertSkuCollections = async (
  ...
  const publishColl = db.collection('publish');

  let sku =
    item.attributes && item.attributes.find(f => f.id == 'SELLER_SKU') ?
      item.attributes.find(f => f.id == 'SELLER_SKU').value_name :
      item.seller_custom_field;

  let skuRet = sku ? await skuColl.findOne({ sku: sku, sellerId: sellerId }) : undefined;
  ...
  if (!sku) {
    ...
    if (!skuXPublish) {
      ...
      throw `O anúncio ${publishId} não possui SKU preenchido e não possui vínculo manual. O anúncio foi removido da base`;      
    }
    sku = skuXPublish.sku;
  }
  ...
  // sincroniza SKU
  await upsertQueueSku(db, 'syncPublishes', [{ sku, sellerId, stock: skuRet.stock, price: skuRet.price }]);
}
...
const syncPublishies = async (db, config, marketPlace, sellerId, publishArray, delIfExists = false, selersByUser = []) => {
  ...
  for (let meliItem of meliItems) {
    ...
    try {
      ...
      let publishQuality;
      if (!meliItem.catalog_listing && meliItem.status != 'inactive') {
      ...
      await publishColl.updateOne(
        ...
        },
        {
          $set: {
            ...
            warrantyTime: meliItem.warranty.split(':').length > 1 ? meliItem.warranty.split(':')[1].trim().split(' ')[0] : '0',
            warrantyTimeUnit: meliItem.warranty.split(':').length > 1 ? meliItem.warranty.split(':')[1].trim().split(' ')[1].trim() : '0',
            updatedAt: new Date(),
            lastSync: new Date(),
            quality: {
