...
const upsertQueueSku = async (db, userName, skus) => {
  
  if (!Array.isArray(skus)) throw `Body isn't valid array`

  let skuXPublishies = 
    await db.collection('skuXPublish').find({ 
      sku: { $in: skus.map(m => m.sku) },
      sellerId: { $in:skus.map(m => m.sellerId) }, 
      publishId: { $not: new RegExp(".*Digi-.*", "i") } 
    }).toArray();
...
module.exports = { upsertQueue, lockAndGetQueue, upsertQueueSku: upsertQueueSku, deleteQueue, upsertQueueError, deleteQueueError, upsertQueueSuccess };