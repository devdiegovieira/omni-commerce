...
router.post('/', async (req, res, next) => {
  ...
  try {
    ...
    let seller = (await getConfig(req.mongoConnection)).seller;

    let cliente = seller.find(e => e.prefixo == req.body.products[0].sku.substr(0, 4));
    let connectionString = cliente ? cliente.invento.connectionString : seller.find(e => e.prefixo == "") ? seller.find(e => e.prefixo == "").invento.connectionString : null;
