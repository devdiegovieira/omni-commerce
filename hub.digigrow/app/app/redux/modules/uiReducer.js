...
import MenuContent from 'digi-api/ui/menu';
import {
  TOGGLE_SIDEBAR,
  OPEN_MENU,
  OPEN_SUBMENU,
...
const initialState = {
  ...
  type: localStorage.getItem('themeType') ? localStorage.getItem('themeType') : 'light', // light or dark
...
const setNavCollapse = (arr, curRoute) => {
  let headMenu = MenuContent.find(f => f.link == curRoute) ? MenuContent.find(f => f.link == curRoute).key : 'not found';
